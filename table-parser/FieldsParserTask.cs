...
namespace TableParser
{
    ...
    public class FieldsParserTask
    {
        ...
        {
            List<Token> listAnswers = new List<Token>();
            ...
            for (var i = 0; i < line.Length; i++)
            {
                ...
                listAnswers.Add(token);
                if (token.Length > 1)
                {
                    i += token.Length - 1;
                }
            }
            return CreateFinalListAnswers(listAnswers);
        }

        public static List<Token> CreateFinalListAnswers(List<Token> listAnswers)
        {
            List<Token> list = new List<Token>();

            foreach (var l in listAnswers)
            {
                if (l.Length > 0)
                {
                    list.Add(l);
